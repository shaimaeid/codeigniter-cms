<?php
class template_parts_model extends CI_Model
{
	...
	function get_Features(){
		$sql = "SELECT * from template_part where template_key='features' limit 0,3";
		$resultsql = $this->db->query($sql)->result_array();
		$images="";
		foreach ($resultsql as $row) 
		{  
	...
	function get_Recent(){
		$sql = "SELECT * from template_part where template_key='recent' ";
		$resultsql = $this->db->query($sql)->result_array();
		$images="";
		foreach ($resultsql as $row) 
		{  
		
			$images.='<li class="portfolio-item four columns jcarousel-item">';
			$images.='<a href="'.$row['url'].'"><img  src="'.ROOT_DIR.'images/recent/'.$row['image'].'" /></a>';
			$images.='<h5>'.$row['title'].'</h5>';
			$images.='<em>'.$row['content'].'</em>';
			$images.='</li>';
	...
	function get_top_menu(){
		$sql = "SELECT * from template_part where template_key='top_menu' ";
		$resultsql = $this->db->query($sql)->result_array();
		$images="";
		foreach ($resultsql as $row) 
		{  
		
			$images.='<a href="'.$row['url'].'">'.$row['title'].'</a> | ';

		}
		return rtrim($images, " | ");
	}	
	function get_footer_menu(){
		$sql = "SELECT * from template_part where template_key='footer_menu' ";
		$resultsql = $this->db->query($sql)->result_array();
		$images="";
		foreach ($resultsql as $row) 
		{  
